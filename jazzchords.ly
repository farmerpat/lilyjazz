...
#(define (JazzChordNames pitch majmin)  ;majmin is a required argument for "chordNamer", but not used here
  ...
  (if (= alt 0) ; alteration ?
    ...
    (if (= alt FLAT) ; flat or sharp
      (markup #:raise 1 #:fontsize -5 #:jazzglyph "accidentals.flatjazz")
      (markup #:raise 1 #:fontsize -5 #:jazzglyph "accidentals.sharpjazz")))))))
...
#(define-markup-command (acMin layout props extension) (string?)
  (interpret-markup layout props
    (markup #:small #:raise 1 "-" #:super extension)))
...
% special chords
  ...
  <c e gis bes>-\markup { \super "7(b5)" } % :7.5-
...
% major third chords - 5+ notes
  ...
  % every one of these?
  % replace me w/t real glyph.. at least this looks like something
  % similar to what is expected
  <c e g bes d' fis'>-\markup { \super "9(#11)" } % :9.11+
  %<c e g bes d' fis'>-\markup { \super {"9(" \musicglyph {#xe126} "11)" }} % :9.11+
  %<c e g bes d' fis'>-\markup {
    %\super {
      %"9("
      %\musicglyph #"accidentals.sharpjazz"
      %"11)"
    %}
  %} % :9.11+
  ...
  %<c e g bes d' fis'>-\markup {
    %\super {
      %9(
      %\musicglyph #"#xe126" %#"accidentals.sharpjazz"
      %11)
    %}
  %} % :9.11+
